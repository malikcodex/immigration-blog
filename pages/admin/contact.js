...
const AllContacts = ({contacts}) => {
    ...
    const [search, searchContacts] = useState(null);

    function search_do() {
        if(search != '') {
            router.push(`/admin/contacts?search=${change_str_to_title(search)}`)
        }
    }

    let get_pagination = getPagination(contacts, page);
    return (
        <Layout title='Contact Management' description='Manage Contact Center' meta='no' keywords='manage contact, messages, help center'>
            <div className="container-fluid">
                <div className="row py-4">
                    <SetSidebar />
                    <div className="col-xl-9 col-md-8 col-sm-7 col-12">
                        <div className="container-fluid">
                            <h3 className="fs-3 fw-bold lh-base pt-3 pb-2">All Contact Messages</h3>
                            <div className="col-xl-6 col-12 mb-4">
                                <div className="input-group bg-white bg-shadow-sm">
                                    <input type="text" className="form-control p-2" autoComplete="off" spellCheck="false" onChange={(e) => searchContacts(e.target.value)} />
                                            ...
                                            <tbody>
                                                {
                                                    ...
                                                    <tr key={k}>
                                                        ...
                                                        <td>{d.date}</td>
                                                        <td>
                                                            <Link href={`/send-message?id=${d._id}`} className="btn btn-primary mx-2 btn-sm">Send</Link>
                                                        </td>
                                                    </tr>
                                                    ))
                                                }
...
export async function getServerSideProps(context) {
     ...
     let contacts = await getAllContacts({search: search}); 
     return {
        props: {
            contacts
        }
