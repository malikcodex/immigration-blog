...
const AllSupportBlogs = ({ articles }) => {
    ...
    return (
        <Layout title="Manage All Blogs" description="Manage All Blogs" meta="no" keywords="Manage Support, Support Center, Need Help?">
            <div className="container-fluid">
                <div className="row py-4">
                    <SetSidebar />
                    <div className="col-xl-9 col-md-8 col-sm-7 col-12">
                        <div className="container-fluid">
                            <h3 className="fs-3 fw-bold lh-base pt-3 pb-2">Manage Blogs / Articles</h3>
                                        ...
                                        <tbody>
                                            {paginatedData.data.map((d, k) => (
                                                <tr key={k}>
                                                    ...
                                                    <td>{nothing_found(d.desc.substring(0, 22) + '...')}</td>
                                                    ...
                                                    <td>
                                                        ...
                                                        <Link href={`/admin/articles?id=${d._id}`} className="btn btn-danger mx-2 btn-sm">
                                                            Delete
                                                        </Link>
                                                    </td>
                                                </tr>
                            ...
                            {paginatedData?.data?.length > 0 && (
                                <div className="input-group py-3">
                                    ...
                                    <Link
                                        href={`/admin/articles?page=${paginatedData.prev_page}`}
                                        className={`btn btn-md bg-green text-white ${paginatedData.prev_disable}`}
                                    >
                                        Prev
                                    </Link>
                                    <Link
                                        href={`/admin/articles?page=${paginatedData.next_page}`}
                                        className={`btn btn-md bg-green text-white ${paginatedData.next_disable}`}
                                    >
                                        Next
                                    </Link>
                                    <Link
                                        href={`/admin/articles?page=${paginatedData.total_pages}`}
                                        className={`btn btn-md bg-green text-white ${paginatedData.next_disable}`}
                                    >
                                        Last
                                    </Link>
...
export async function getServerSideProps(context) {
    ...
    const articles = await getAllArticles({ id, category:'blog', search });
    return {
        props: {
            articles: articles,
        }
