...
const Articles = ({articles}) => {
    ...
    function set_search_btn(e) {
        ...
        if(search != 0 && search != null) {
            router.push(`/admin/support/${search}`);
        } else {
    ...
    return (
        <Layout title='Manage All Support Center' description='Manage All Support Center' meta='yes' keywords='handle blogs, list articles'>
            <div className="container-fluid">
                <div className="row py-4">
                    <SetSidebar />
                    <div className="col-xl-9 col-md-8 col-sm-7 col-12">
                        <div className="container-fluid">
                            <h3 className="fs-3 fw-bold lh-base pt-3 pb-2">Manage All Support Center</h3>
                            <Link href="/admin/add-articles" className="text-white btn btn-sm bg-dark mb-3">Add Support</Link>
                                            ...
                                            <tbody>
                                                {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <Link href={`/admin/support?id=${d._id}`} className="btn btn-danger mx-2 btn-sm">Delete</Link>
                                                        </td>
                                                    </tr>
                                                    ))
                                                }
                                    ...
                                    <div className="input-group py-3">
                                        {
                                            get_pagination.data && get_pagination.data.length > 0 && (
                                                <>
                                                <Link href={`/admin/support?page=1`} className={`btn btn-md bg-green text-white ${get_pagination.prev_disable}`}>First</Link>
                                                <Link href={`/admin/support?page=${get_pagination.prev_page}`} className={`btn btn-md bg-green text-white ${get_pagination.prev_disable}`}>Prev</Link>
                                                <Link href={`/admin/support?page=${get_pagination.next_page}`} className={`btn btn-md bg-green text-white ${get_pagination.next_disable}`}>Next</Link>
                                                <Link href={`/admin/support?page=${get_pagination.next_page}`} className={`btn btn-md bg-green text-white ${get_pagination.next_disable}`}>Last</Link>
                                                </>
                                            )
                                        }
                                    </div>
...
export async function getServerSideProps(context) {
    ...
    let articles = await getAllArticles({id: id, category: 'support'});
    return {
        props: {
            articles
        }
